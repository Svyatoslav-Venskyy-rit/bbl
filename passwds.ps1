<#
.SYNOPSIS
    Master IRSEC Account Script: Combined AD/Local Management.
    
.DESCRIPTION
    1. Resets AD passwords for whitelisted users (excluding whiteteam, etc.).
...
$keepEnabledList = @(
    ...
    "terminator", "mrpeabody", "jamescole", "docbrown", "professorparadox",
    "administrator", "krbtgt", "whiteteam", "datadog", "dd-dog", "blackteam"
)

# 2. Accounts that should NEVER have their password changed
$pwResetExclusionList = @(
    "whiteteam", "datadog", "dd-dog", "blackteam", "krbtgt", "administrator"
)
...
Write-Host "`n✅ Master Report saved to: $OutputMappingPath" -ForegroundColor Green
